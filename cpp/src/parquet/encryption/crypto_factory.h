...
/// The ExternalEncryptionConfiguration will use this to send encryption parameters per
/// column.
...
struct PARQUET_EXPORT ExternalEncryptionConfiguration : public EncryptionConfiguration {
  ...
  /// Map of the columns to encrypt to their associated encryption parameters. The id of
  /// the map is the column name, and the value is a ColumnEncryptionAttributes struct
  /// that can be used to construct the ColumnEncryptionProperties in the CryptoFactory.
  ...
  /// If a column name appears in the original column_keys list, it will be encrypted with
  /// the algorithm specified in the encryption_algorithm field. If a column name appears
  /// in the new per_column_encryption map, it will be encrypted using the per column
  /// specific algorithm and key. If a column name appears in both, an exception will be
  /// thrown.
  ...
  /// enforce robust access control. The values sent to the external encryptor depend on
  /// each implementation. This value must be a valid JSON-formatted string. Validation of
  /// the string will be done by the external encryptor, Arrow will only forward this
  /// value. Format: "{\"user_id\": \"abc123\", \"location\": {\"lat\": 9.7489, \"lon\":
  /// -83.7534}}"
  std::string app_context;

  /// Map of the encryption algorithms to the key/value map of the location of
  /// configuration files needed by the external encryptors. This may include location of
  /// a dynamically-linked library, or the location of a file where the external encryptor
  /// can find urls, certificates, and parameters needed to make a remote call. For
  /// security, these values should never be sent in this config, only the locations of
...
struct PARQUET_EXPORT ExternalDecryptionConfiguration : public DecryptionConfiguration {
  ...
  /// enforce robust access control. The values sent to the external decryptor depend on
  /// each implementation. This value must be a valid JSON-formatted string. Validation of
  /// the string will be done by the external decryptors, Arrow will only forward this
  /// value. Format: "{\"user_id\": \"abc123\", \"location\": {\"lat\": 9.7489, \"lon\":
  /// -83.7534}}"
  std::string app_context;

  /// Map of the encryption algorithms to the key/value map of the location of
  /// configuration files needed by the external decryptors. This may include location of
  /// a dynamically-linked library, or the location of a file where the external decryptor
  /// can find urls, certificates, and parameters needed to make a remote call. For
  /// security, these values should never be sent in this config, only the locations of
...
class PARQUET_EXPORT CryptoFactory {
 public:
  ...
  /// Get the external decryption properties for a Parquet file. Used when an external
  /// decryptor will be used to decrypt the file.
