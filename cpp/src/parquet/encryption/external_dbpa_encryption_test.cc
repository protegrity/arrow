...
/// encryptor and decryptor are created and that the plaintext is returned as the
/// ciphertext.
...
class ExternalDBPAEncryptorAdapterTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    connection_config_ = {{"lib_name", "dbpa_lib.so"}, {"config_path", "path/to/file"}};
  }
  ...
  void RoundtripEncryption(ParquetCipher::type algorithm, std::string column_name,
    ...
    int32_t encryption_length =
        encryptor.Encrypt(str2span(plaintext), str2span(empty_string),
                          str2span(empty_string), ciphertext_buffer);
    ...
    int32_t decryption_length =
        decryptor.Decrypt(str2span(ciphertext_str), str2span(empty_string),
                          str2span(empty_string), plaintext_buffer);
