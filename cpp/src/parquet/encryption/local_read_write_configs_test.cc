...
class LocalReadWriteEncryption : public ::testing::Test {
 protected:
...
TEST_F(LocalReadWriteEncryption, CTR_WriteRead) {
...
TEST_F(LocalReadWriteEncryption, PerColumnExternal_WriteRead) {
  ...
  file_decryption_builder.connection_config({{parquet::ParquetCipher::EXTERNAL_DBPA_V1,
                                              {{"agent_library_path", library_path},
                                               {"file_path", "/tmp/test"},
                                               {"other_config", "value"}}}});
