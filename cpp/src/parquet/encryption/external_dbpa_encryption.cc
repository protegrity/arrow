...
std::unique_ptr<dbps::external::DataBatchProtectionAgentInterface> LoadAndInitializeAgent(
  ...
  try {
    if (configuration_properties.find(INIT_TIMEOUT_KEY) !=
        configuration_properties.end()) {
      init_timeout_ms = std::stoi(configuration_properties.at(INIT_TIMEOUT_KEY));
    }
    if (configuration_properties.find(ENCRYPT_TIMEOUT_KEY) !=
        configuration_properties.end()) {
      encrypt_timeout_ms = std::stoi(configuration_properties.at(ENCRYPT_TIMEOUT_KEY));
    }
    if (configuration_properties.find(DECRYPT_TIMEOUT_KEY) !=
        configuration_properties.end()) {
      decrypt_timeout_ms = std::stoi(configuration_properties.at(DECRYPT_TIMEOUT_KEY));
    }
  } catch (const std::exception& e) {
    ARROW_LOG(ERROR) << "Failed to parse timeout values from configuration_properties: "
                     << e.what();
    throw ParquetException(
        "Failed to parse timeout values from configuration_properties");
  }
  ...
  // Convert KeyValueMetadata (only provided by the decryptor) into a
...
std::unique_ptr<ExternalDBPAEncryptorAdapter> ExternalDBPAEncryptorAdapter::Make(
  ...
  auto agent_instance =
      LoadAndInitializeAgent(column_name, configuration_properties, app_context, key_id,
  ...
  ARROW_LOG(DEBUG) << "ExternalDBPAEncryptorAdapter::ExternalDBPAEncryptorAdapter() -- "
                      "creating ExternalDBPAEncryptorAdapter";

  // create the instance of the ExternalDBPAEncryptorAdapter
  auto result =
      std::unique_ptr<ExternalDBPAEncryptorAdapter>(new ExternalDBPAEncryptorAdapter(
          /*algorithm*/ algorithm,
          /*column_name*/ column_name,
          /*key_id*/ key_id,
          /*data_type*/ data_type,
...
std::shared_ptr<KeyValueMetadata> ExternalDBPAEncryptorAdapter::GetKeyValueMetadata(
  ...
  std::unordered_map<std::string, std::string> unordered_map(metadata_map.begin(),
                                                             metadata_map.end());
...
int32_t ExternalDBPAEncryptorAdapter::EncryptWithManagedBuffer(
    ...
    std::unique_ptr<EncodingProperties> encoding_properties, int8_t module_type) {
...
int32_t ExternalDBPAEncryptorAdapter::InvokeExternalEncrypt(
  ...
  ARROW_LOG(DEBUG) << "  result ciphertext size: " << result->ciphertext().size()
                   << " bytes";
...
ExternalDBPAEncryptorAdapter* ExternalDBPAEncryptorAdapterFactory::GetEncryptor(
  ...
  if (encryptor_cache_.find(column_path->ToDotString()) == encryptor_cache_.end()) {
    auto configuration_properties =
        external_file_encryption_properties->configuration_properties();
    if (configuration_properties.find(algorithm) == configuration_properties.end()) {
      throw ParquetException(
          "External DBPA encryption requires its configuration properties");
    }
...
std::unique_ptr<ExternalDBPADecryptorAdapter> ExternalDBPADecryptorAdapter::Make(
  ...
  auto result =
      std::unique_ptr<ExternalDBPADecryptorAdapter>(new ExternalDBPADecryptorAdapter(
          /*algorithm*/ algorithm,
          /*column_name*/ column_name,
          /*key_id*/ key_id,
          /*data_type*/ data_type,
...
int32_t ExternalDBPADecryptorAdapter::PlaintextLength(int32_t ciphertext_len) const {
  throw ParquetException(
      "ExternalDBPADecryptorAdapter::PlaintextLength is not supported");
}
...
int32_t ExternalDBPADecryptorAdapter::InvokeExternalDecrypt(
  ...
  std::unique_ptr<DecryptionResult> result =
      agent_instance_->Decrypt(ciphertext, std::move(encoding_attrs));
  ...
  ARROW_LOG(DEBUG) << "  result size: " << result->size() << " bytes";
  ARROW_LOG(DEBUG) << "  result plaintext size: " << result->plaintext().size()
                   << " bytes";
...
std::unique_ptr<DecryptorInterface> ExternalDBPADecryptorAdapterFactory::GetDecryptor(
  ...
  if (column_chunk_metadata == nullptr || crypto_metadata == nullptr) {
    throw ParquetException(
        "External DBPA decryption requires column chunk and crypto metadata");
  }
  auto configuration_properties =
      external_file_decryption_properties->configuration_properties();
  if (configuration_properties.find(algorithm) == configuration_properties.end()) {
    throw ParquetException(
        "External DBPA decryption requires its configuration properties");
  }
  auto column_path = column_chunk_metadata->descr()->path();
  auto data_type = column_chunk_metadata->descr()->physical_type();
  std::optional<int> datatype_length;
